...
var arremail = [];
var MarkFinalContractID = "";
var MarkFinalContractType = "";
var ContractRoles = [];
...
$(document).ready(function () {
    ...
    //ENH 452 Added Cancel Status in Pipeline
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (thisContractAreaName != "") {
                    ...
                    success: function (data1) {
                        for (var i = 0; i < datalenght; i++) {
                            ...
                            if (data1.ContractType.trim().split(';').indexOf(item.ContractType) > -1) {
                                var myUrl = '/Contracts/CreateContract?Stage=pipeline&ContractType=' + encodeURIComponent(item.ContractType) + "&DocumentCreation=" + encodeURIComponent(item.DocumentCreation);
                                var article = '<li><a href=' + myUrl + ' style="word-break: break-all;">' + item.ContractType + '</a></li>';
                ...
                for (var i = 0; i < datalenght; i++) {
                    var item = data[i];
                    var myUrl = '/Contracts/CreateContract?Stage=pipeline&ContractType=' + encodeURIComponent(item.ContractType);
                    var article = '<li><a href=' + myUrl + ' style="word-break: break-all;">' + item.ContractType + '</a></li>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < datalenght; i++) {
                ...
                var myUrl = '/Contracts/CreateContract?ContractType=' + encodeURI(item.ContractType) + '&ContractTemplateID=' + item.RowKey + "&Stage=pipeline";
                var article = '<li><a href=' + myUrl + ' style="word-break: break-all;">' + item.TemplateName + '</a></li>';
...
$(document).ready(function () {
    ...
    if (vViewName == 'All Contracts in Pipeline' || vViewName == '' || vViewName == "All") {
    ...
    $("#browseCounterparty").dialog({
        ...
        modal: true,
        buttons: {
            "OK": function () { ChangeCounterpartyMultiple(); multipleChecksDocumentID = []; },
            Cancel: function () {
...
function GetSavedViews() {
    ...
    //Get Contract views
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/savedview?viewfor=PipelineContract&userid=',
        type: 'GET',
        dataType: 'json',
        'Content-Type': 'application/json',
        cache: false,
        ...
        success: function (data) {
            ...
            if (showAfterSave == "add" && typeof showAfterSave != "undefined") {
                if (personData != "") {
                    var viewObj = $("#liContractViews li p a").filter('[id*=' + '"' + personData.split('&')[0] + '"' + ']');
                    $("#loadingPage").fadeOut();
                    if (viewObj.length > 0)
       ...
       if (confirmed) {
           ...
           $.ajax({
               ...
               success: function (data) {
                   ...
                   var vViewName = "";
                   vViewName == 'All Contracts in Pipeline';

                   var obj = { name: "All", innerText: "All Contracts in Pipeline" };
                   quickViewDisplay(obj);
...
function CreateContractList(page) {
    ...
    for (var j = startIndex; j < endIndex; j++) {
        ...
        if (item != null) {
            ...
            if (item.InRecycleBin == "Yes") {
                ...
                if (item.Permission == 'Manage') {
                    vPermission = 'openmenuRecycleDelete';
                }
                else {
            ...
            var myUrl = '/Contracts/ContractDetails?ContractID=' + encodeURI(item.RowKey) + '&Stage=pipeline&View=' + ObjectNameToSend;
            ...
            if (item.InRecycleBin == "Yes") {
                ...
                if (ObjectNameToSend == "MySavedDrafts" || item.IsDraft == "Yes" || ObjectNameToSend == "Draft") {
                    ...
                    if (item.Status == "New") {
                        ...
                        if (vPermission == 'openmenuContribute' || vPermission == 'openmenu')
                            vQuickActions += '<a href="javascript:void(0)" onclick="ManageContractStatus(\'' + item.RowKey + '\',\'' + item.Status + '\');"><img title="Manage Status" src="../Content/Images/action/manage-status.png"/></a>';
                        if (vPermission == 'openmenu' || vPermission == 'openmenuDraft') {
                            vQuickActions += '<a href="javascript:void(0)" onclick="ManageRelatedContracts(\'' + item.RowKey + '\',\'' + item.ContractTitle + '\', \'' + escape(item.BusinessAreaPath) + '\');"><img title="Tag to Replaced / Related Contract" src="../Content/Images/action/tag-to-replace.png"/></a>';
                        }
                    }
                        ...
                        if (vPermission == 'openmenuContribute' || vPermission == 'openmenu' || vPermission == 'openmenuStatus' || vPermission == 'openmenuStatusContribute') {
                            if (vPermission == 'openmenuStatusContribute') {
                                vPermission = 'openmenuStatusContributeCancel';
                            }
                    ...
                    if (vPermission == 'openmenuContribute' || vPermission == 'openmenu' || vPermission == 'openmenuDraft' || vPermission == 'openmenuDraftContribute') {
                        vQuickActions += '<a href="/Contracts/EditContract?ContractID=' + item.RowKey + '&ContractType=' + encodeURIComponent(item.ContractType) + '&Stage=pipeline"><img title="Edit Metadata" src="../Content/Images/action/edit-metadata.png"/></a>';
                        if (vPermission == 'openmenuContribute' || vPermission == 'openmenu') {
                            vQuickActions += '<a href="javascript:void(0)" onclick="OpenContractApproval(\'' + item.ApprovalWorkflow + '\', \'' + escape(item.ContractTitle) + '\', \'' + item.RowKey + '\', \'' + escape(item.BusinessArea) + '\', \'' + escape(item.ContractArea) + '\', \'' + escape(item.BusinessAreaPath) + '\');"><img title="Start Approval Workflow" src="../Content/Images/action/Start-Approval-workflow.png"/></a>';
                        }
                    }
                    vPermission = vPermissionWoDel;
                }
                    ...
                    vQuickActions += '<a href="' + myUrl + '&Tab=Activity" style="word-break: break-all;"><img title="Check Activity Status" src="../Content/Images/action/check-activity-status.png"/></a>';
                    vQuickActions += '<a href="' + myUrl + '&Tab=Notes" style="word-break: break-all;"><img title="View Notes" src="../Content/Images/action/view-notes.png"/></a>';
                    vPermission = vPermissionWoDel;
                }
                else if (item.Status == "Negotiation Complete") {
                    ...
                    vQuickActions += '<a href="javascript:void(0)" onclick="CreateContractActivityList(\'' + item.RowKey + '\');"><img title="View History" src="../Content/Images/action/View-History.png"/></a>';
                    if (vPermission == 'openmenuContribute' || vPermission == 'openmenu' || vPermission == 'openmenuStatus' || vPermission == 'openmenuStatusContribute') {//ENH 452 Added two more Permissions when Pipeline Status is in Cancel
                        if (vPermission == 'openmenuStatusContribute') {
                            vPermission = 'openmenuStatusContributeCancel';
                        }
                        vQuickActions += '<a href="javascript:void(0)" onclick="ManageContractStatus(\'' + item.RowKey + '\',\'' + item.Status + '\');"><img title="Manage Status" src="../Content/Images/action/manage-status.png"/></a>';
    ...
    //ENH 452 Added two more contextMenu when Pipleine Status is in Cancel
    ...
    $(".openmenuStatusContributeCancel").contextMenu({ menu: 'myMenuStatusContributeCancel', leftButton: true }, function (action, el, pos) { contextMenuWork(action, el.parent("div").parent("li"), pos); });
    ...
    $(".openmenuRecycleDelete").contextMenu({ menu: 'myMenuRecyclebinDelete', leftButton: true }, function (action, el, pos) { contextMenuWorkForRecycleBin(action, el.parent("div").parent("li"), pos); });
    ...
    $(".openmenuRecycleDelete").click(function () {
        $("#dvfilter").hide();
    });
...
function SelectAllItems(object) {
    ...
    if (multipleChecks.trim() == "") {
        ...
        if (multipleChecksPermission.indexOf('openmenuView') >= 0) {
            ...
            if (typeof localStorage.GlobalBusinessAreaLocation != "undefined" && localStorage.GlobalBusinessAreaLocation != "" && localStorage.GlobalBusinessAreaLocation != "All")
                $("#btnMultipleAction_BusnArea").css('display', '');
            if (!(multipleChecksStatus.indexOf("New") >= 0 || multipleChecksStatus.indexOf("Awaiting Approval") >= 0 || multipleChecksStatus.indexOf("Approved") >= 0 || multipleChecksStatus.indexOf("Awaiting Review") >= 0 || multipleChecksStatus.indexOf("Reviewed") >= 0 || multipleChecksStatus.indexOf("In Negotiation") >= 0 || (multipleChecksIsDraftByCurrent.indexOf("No") >= 0)))
    ...
    if (ObjectNameToSend == "MySavedDrafts" || ObjectNameToSend == "Draft") {
        $("#btnMultipleAction_Status").css('display', 'none');
...
function CreateContractListOld(page) {
    $(".drop_a").hide();
    multipleChecks = "";
    var startIndex = page * 20;
    var endIndex = startIndex + 20;
    ...
    for (var j = startIndex; j < endIndex; j++) {
        ...
        if (item != null) {
            ...
            article += '<p id="BaseContractValueCurrency" style="display:none;">' + item.BaseContractValueCurrency + '</p>';
            if (item.InRecycleBin == "Yes") {
                article += '<p class="margin-left-20"><i><a href=javascript:void(0) class="PreserveSpace">' + item.ContractTitle + '</a>';
            } else {
                article += '<p> <input name="" type="checkbox" onclick="checkMultipleContracts(this);" id=' + item.RowKey + ' value="" /><i><a href=' + myUrl + ' style="word-break: break-all;" class="PreserveSpace">' + item.ContractTitle + '</a><img src="../Content/Images/context_Menu.png" alt="Open Menu" title="Open Menu" class="' + vPermission + '" />';
            }
                        ...
                        if (index == 0) {
                            ...
                            switch (fieldtype) {
                                ...
                                case "User":
                                    ...
                                    break;
                                case "Yes/No":
                                    temp = $("#" + $(val).attr('id') + " select").filter("[id*='YesNo_value_']").val();
                                    if (typeof (temp) == 'undefined' || temp == 'undefined' || temp == null)
                                        temp = "";
                                    query += ' <value>' + $.trim(temp) + '</value>';
                                    break;
                                case "Value / Financials":
                                    temp = $("#" + $(val).attr('id') + " input:text").filter("[id*='num_value_']").val()
                                    if (typeof (temp) == 'undefined' || temp == 'undefined' || temp == null)
                                        temp = "";
                                    query += ' <value>' + $.trim(temp) + '</value>';
                                    break;
...
$("#ddlRule").change(function (obj) {
    ...
    if (vFilterRule.length > 0) {
        ...
        if ($("#ddlRule option:selected").text() == "Default") {
            ...
            if (typeof workflowRules.WorkflowTitle != 'undefined' && workflowRules.WorkflowTitle != "") {
                $("#txtWorkflowTitle").val(workflowRules.WorkflowTitle + ' for ' + $("#ddlRule option:selected").text());
            } else {
                $("#txtWorkflowTitle").val($("#ddlRule option:selected").text());
            }
        }
...
function contextMenuWorkOld(action, el, pos) {
        ...
        case "edit":
            {
                ...
                location = "/Contracts/EditContract?ContractID=" + entityid + '&ContractType=' + encodeURIComponent(contracttype) + "&Stage=pipeline";
                break;
            }
        case "viewdetails":
            {
                var entityid = $(el).find("#ContractID").text();
                location = "/Contracts/ContractDetails?ContractID=" + entityid + "&Stage=pipeline&View=" + ObjectNameToSend;
                break;
            }
        case "approve":
            {
                        ...
                        success: function (item) {
                            ...
                            if (vWorkflowRules.length > 0) {
                                ...
                                if (workflowRules.RuleName == "Default") {
                                    ...
                                    if (typeof workflowRules.WorkflowTitle != 'undefined' && workflowRules.WorkflowTitle != "") {
                                        $("#txtWorkflowTitle").val(workflowRules.WorkflowTitle + ' for ' + workflowRules.RuleName);
                                    } else {
        ...
        case "share":
            {
                ...
                $.ajax({
                    ...
                    success: function (settings) {
                        ...
                        $("#lblValidLinkDateContractPipeline").html(moment(new Date()).add(settings.TaskDuration, "days").format('MM/DD/YYYY'));
                    },
                    error: function () {
        ...
        case "status":
            {
                ...
                $('input:radio[name="PipelineStatus"][value="' + Status + '"]').prop('checked', true);
...
function contextMenuWork(action, el, pos) {
    ...
    switch (action) {
        ...
        case "approve":
            {
                ...
                //manoj
                OpenContractApproval(approvalWorkflow, contractTitle, contractID, businessArea, contractArea, businessAreaPath, contractType, contractDocumentURL, contractSummaryDocumentURL, varContractStatus, varContractPermission);
                break;
            }
        case "review":
            {
        ...
        case "status":
            {
                var contractID = $(el).find("#ContractID").text();
                MarkFinalContractID = contractID;
                MarkFinalContractType = $(el).find("#ContractType").text();
                var status = $(el).find("#Status").text();
                $("#hdContractID").val(contractID);
...
function OpenContractApproval(approvalWorkflow, contractTitle, contractID, businessArea, contractArea, businessAreaPath, contractType, contractDocumentURL, vSummeryBlobURL, vConItemStatus, vConItemPermission) {
    ...
    if (approvalWorkflow == "In Progress") {
        ...
        $("#txtWorkflowTitle").val('Approval for ' + contractTitle);
        $("#txtDuration").val("");
        if (!PipeIndexApprovalTextarea) {
            PipeIndexApprovalTextarea = new nicEditor({ buttonList: ['bold', 'italic', 'underline', 'strikethrough', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'removeformat', 'hr', 'indent', 'outdent', 'link', 'unlink', 'forecolor', 'bgcolor', 'xhtml'] }).panelInstance('txtComment');
            $('.nicEdit-panelContain').parent().width('100%');
            $('.nicEdit-panelContain').parent().next().width('100%');
            $('.nicEdit-main').width("99%");
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < datalenght; i++) {
                ...
                $("#ddlSendToCancel").append(article);//ENH 452 Added Cancel Status in Pipeline
            ...
            $("#ddlSendToCancel").chosen();//ENH 452 Added Cancel Status in Pipeline
...
function CreateContractActivityList(contractid, obj) {
    $("#hdContractID").val(contractid);
    $("#loadingPage").fadeIn();
    var vUrl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/activities/contracts/' + $("#hdContractID").val() + '?actiontype=';
    if (obj !== undefined) {
        vUrl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/activities/contracts/' + $("#hdContractID").val() + '?actiontype=&objectname=' + obj;
    }
    else {
        $("#ddlHistoryFilter").val($("#ddlHistoryFilter option:first").val());
    }
...
function ClearShareForm() {
    ...
    $("#chkDiscloseCon").prop('checked', false);
    var vSignee = '<tr>';
    vSignee += '<td>';
...
function ChangeContractStatus() {
    if (requiredValidator("dvManageContractStatus", false)) {
        ...
        var vStatusNote = '';
        //ENH 452 Added Cancel Status in Pipeline
        if (stat == "Cancelled" && $("#hdContractStatus").val() != "Cancelled") {
            $("#loadingPage").fadeOut();
        ...
        else if (stat == $("#hdContractStatus").val()) {
            $("#loadingPage").fadeOut();
...
function MarkContractAsFinal() {
    ...
    var metadataFields = [];
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/IContractDetails?contractid=' + MarkFinalContractID,
        type: 'GET',
        dataType: 'json',
        headers: { 'eContracts-ApiKey': localStorage.APIKey },
        cache: false,
    ...
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/contracttypes/metadatas?contracttypename=' + MarkFinalContractType,
        type: 'GET',
        dataType: 'json',
        headers: { 'eContracts-ApiKey': localStorage.APIKey },
        cache: false,
    ...
    if (!requiredavalible) {
        swal({
            title: '',
            text: "Are you sure you want to <span style=\"font-weight:700\">mark</span> this contract as <span style=\"font-weight:700\">Finalized/Ready for Signature</span>?",
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes',
            cancelButtonText: 'No',
     ...
     function (confirmed) {
         if (confirmed) {
             $("#loadingPage").fadeIn();
             $.ajax({
                 url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?contractid=' + $("#hdContractID").val() + '&finalized=Yes&username=' + localStorage.UserName + '&docs=' + vdocs,
                 type: 'PUT',
                 dataType: 'json',
                 headers: { 'eContracts-ApiKey': localStorage.APIKey, 'eContracts-RestrictionHeader': localStorage.RestrictHighSecurityTagging },
                 cache: false,
                 success: function (data) {
                     swal("", "This Contract Record is moved to Finalized Contracts. Check 'Contracts' tab.");
                     $("#dvManageContractStatus").dialog("close");
                     $("#loadingPage").fadeOut();
                     applyFilter();
                 },
        ...
        function (confirmed) {
            if (confirmed) {
                if (vdocs == "Yes") {
                    location = "/Contracts/EditContract?ContractID=" + MarkFinalContractID + "&ContractType=" + MarkFinalContractType + "&Stage=pipeline&Finalize=true&DocFianlize=" + vdocs;
                } else {
                    location = "/Contracts/EditContract?ContractID=" + MarkFinalContractID + "&ContractType=" + MarkFinalContractType + "&Stage=pipeline&Finalize=true";
                }
            } else {
                MarkFinalContractID = '';
                MarkFinalContractType = '';
                $("#loadingPage").fadeOut();
                $("#dvManageContractStatus").dialog("close");
            }
        });
...
$("#ddlContractManagers").change(function () {
    ...
    if (contractmanagers != null) {
        ...
        if (arrDiff == null) {
            swal("", arrDiff + " " + "items are already there");
...
function savePeople() {
    ...
    if (requiredValidator('addNewPeople')) {
        ...
        if ($("#chkpermission").is(':checked')) {
            ...
            status = $("#chkpermission").is(':checked') ? 'Yes' : 'No';
            var vIsPublic = $('#chkpubliccontract').is(':checked') ? 'Yes' : 'No'; //eO310627
            var formData = new FormData();
            formData.append("ContractManagers", cm);
            formData.append("PermissionAssignment", "Role-Based");
            formData.append("Reviewers", rev);
                ...
                if (savedviewentity != null && savedviewentity != "") {
                    ...
                    switch (selectedSortOption) {
                        ...
                        case "Renewal Date":
                            ...
                            break;
                        case "Expiration Date":
                            sortby = '&sortbyfield=ExpiryDate&orderby=DESC';
                            break;
                        case "Contract Value(Low-High)":
                    ...
                    newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?BusinessAreaPath=' + baloc + '&stage=pipeline&filterXml=' + viewQueryXml + sortby + "&viewName=" + qvName + '&searchkeyword=' + encodeURIComponent($("#txtSearchBox").val());
                    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
                    $.ajax({
                        url: newurl,
                        type: 'GET',
...
function GeContractsSavedAsDraft() {
    ...
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/savedasdraftpipeline',
        type: 'GET',
        dataType: 'json',
        'Content-Type': 'application/json',
        cache: false,
...
function BindContractStatusFilter() {
    $.ajax({
        ...
        success: function (contractstatuses) {
            ...
            $(contractstatuses).each(function (i, item) {
                ...
                if (item.ContractStatus == "Ready for Signature" || item.ContractStatus == "Awaiting Signatures" || item.ContractStatus == "Signed") { } else {
    ...
    $('#tdPipelineStatus').empty();
    $('#tdMulPipelineStatusMul').empty();
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/contractstatusesbyCLM',
        type: 'GET',
        dataType: 'json',
                ...
                if ((item.Transition == "Manual" || contractItem.Status == item.ContractStatus) && item.Active == true) {
                    ...
                    if (item.ContractStage == "Pipeline") {
                        ...
                        if (item.ContractStatus == "New") {
                            ctrl = "<input id='rdPipelineNew' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='New: The Contract Record just got initiated.' class='status_green_another'><img src='../Content/Images/status/new.png'>new</b><label for='rdPipelineNew' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineNew' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='New: The Contract Record just got initiated.' class='status_green_another'><img src='../Content/Images/status/new.png'>new</b><label for='rdPipelineNew' class='css-label'>" + item.ContractStatus + "</label><br />";

                        }
                        else if (item.ContractStatus == "Awaiting Review") {
                            ctrl = "<input id='rdPipelineAwaitReview' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Awaiting Review:The associated contract document is waiting to be reviewed by the Internal Reviewers. ' class='status_yellow'><img src='../Content/Images/status/renew.png'>rev</b><label for='rdPipelineAwaitReview' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineAwaitReview' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Awaiting Review:The associated contract document is waiting to be reviewed by the Internal Reviewers. ' class='status_yellow'><img src='../Content/Images/status/renew.png'>rev</b><label for='rdPipelineAwaitReview' class='css-label'>" + item.ContractStatus + "</label><br />";

                        }
                        else if (item.ContractStatus == "Reviewed") {
                            ctrl = "<input id='rdPipelineReviewed' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Reviewed:The Internal Reviewers have reviewed the associated contract document.' class='status_blue'><img src='../Content/Images/status/renew.png'>rev</b><label for='rdPipelineReviewed' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineReviewed' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Reviewed:The Internal Reviewers have reviewed the associated contract document.' class='status_blue'><img src='../Content/Images/status/renew.png'>rev</b><label for='rdPipelineReviewed' class='css-label'>" + item.ContractStatus + "</label><br />";


                        }
                        else if (item.ContractStatus == "Awaiting Approval") {
                            ctrl = "<input id='rdPipelineAwaitApproval' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Awaiting Approval:  The Contract Record is waiting to be approved by the Internal Approvers. ' class='status_yellow'><img src='../Content/Images/status/renew.png'>appr</b><label for='rdPipelineAwaitApproval' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineAwaitApproval' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Awaiting Approval:  The Contract Record is waiting to be approved by the Internal Approvers. ' class='status_yellow'><img src='../Content/Images/status/renew.png'>appr</b><label for='rdPipelineAwaitApproval' class='css-label'>" + item.ContractStatus + "</label><br />";


                        }
                        else if (item.ContractStatus == "Approved") {
                            ctrl = "<input id='rdPipelineApproved' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Approved:The Internal Approvers have approved the Contract Record.' class='status_blue'><img src='../Content/Images/status/tick.png'>appr</b><label for='rdPipelineApproved' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineApproved' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Approved:The Internal Approvers have approved the Contract Record.' class='status_blue'><img src='../Content/Images/status/tick.png'>appr</b><label for='rdPipelineApproved' class='css-label'>" + item.ContractStatus + "</label><br />";


                        }
                        else if (item.ContractStatus == "In Negotiation") {
                            ctrl = "<input id='rdPipelineInNegotiation' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='In Negotiation:The Contract Record is waiting to be negotiated with the counterparty.' class='status_yellow'><img src='../Content/Images/status/renew.png'>nego</b><label for='rdPipelineInNegotiation' class='css-label'>" + item.ContractStatus + "</label><br />";

                            ctrlMul = "<input id='rdPipelineInNegotiation' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='In Negotiation:The Contract Record is waiting to be negotiated with the counterparty.' class='status_yellow'><img src='../Content/Images/status/renew.png'>nego</b><label for='rdPipelineInNegotiation' class='css-label'>" + item.ContractStatus + "</label><br />";

                        }

                        else if (item.ContractStatus == "Negotiation Complete") {
                            ctrl = "<input id='rdPipelineNegotiationComplete' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Negotiation Complete:The negotiation with the counterparty for this Contract Record is complete.' class='status_blue'><img src='../Content/Images/status/tick.png'>nego</b><label for='rdPipelineNegotiationComplete' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineNegotiationComplete' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Negotiation Complete:The negotiation with the counterparty for this Contract Record is complete.' class='status_blue'><img src='../Content/Images/status/tick.png'>nego</b><label for='rdPipelineNegotiationComplete' class='css-label'>" + item.ContractStatus + "</label><br />";


                        }
                            //ENH 452 Added Cancel Status in Pipeline
                        else if (item.ContractStatus == "Cancelled") {
                            ctrl = "<input id='rdPipelineCancelled' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Aborted:The Contract Record is Cancelled.' class='status_Gray'><img src='../Content/Images/status/close.png'>canc</b><label for='rdPipelineCancelled' class='css-label'>" + item.ContractStatus + "</label><br />";
                            ctrlMul = "<input id='rdPipelineCancelled' type='radio' name='PipelineStatusMul' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='The Contract Record is Aborted  with the counterparty.' class='status_Gray'><img src='../Content/Images/status/close.png'>canc</b><label for='rdPipelineCancelled' class='css-label'>" + item.ContractStatus + "</label><br />";

                        }

                        $("#tdPipelineStatus").append(ctrl);
                        $("#tdMulPipelineStatusMul").append(ctrlMul);
                    }
                }
            }
        }
                ...
                if ((item.Transition == "Manual" || status == item.ContractStatus) && item.Active == true) {
                    ...
                    if (item.ContractStage == "Pipeline") {
                        ...
                        if (item.ContractStatus == "New") {
                            ...
                            ctrl = "<input id='rdPipelineCancelled' type='radio' name='PipelineStatus' value='" + item.ContractStatus + "' class='css-checkbox " + vAutoChange + "' /><b title='Cancelled:The Contract Record is Complete Cancelled with the counterparty ' class='status_Gray'><img src='../Content/Images/status/close.png'>canc</b><label for='rdPipelineCancelled' class='css-label'>" + item.ContractStatus + "</label><br />"
                        }
                        $("#tdPipelineStatus").append(ctrl);
                    }
                }
...
function GetNewDraftContracts(objectname) {
    $("#spResult").empty();
    var sortby = '&sortbyfield=Created&orderby=DESC';
    var baname = ";";
    if (thisBusinessAreaName == "" || thisBusinessAreaName == "All") {
    } else {
        baname = thisBusinessAreaName;
    ...
    var sortby = '&sortbyfield=Timestamp&orderby=DESC';
    var newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?stage=pipeline&searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery + sortby;
    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
    $.ajax({
        url: newurl,
        type: 'GET',
...
function GetCreatedByMe() {
    ...
    var customQuery = "CreatedBy:" + localStorage.UserName + ",";
    var sortby = '&sortbyfield=Timestamp&orderby=DESC';
    var newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?stage=Pipeline&searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery + sortby;
    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
    $.ajax({
...
function GetAwaitingSignatures() {
    $("#spResult").empty();
    var baname = ";";
    if (thisBusinessAreaName == "" || thisBusinessAreaName == "All") {
    } else {
    ...
    var customQuery = "Status:Awaiting Signatures";
    var sortby = '&sortbyfield=Timestamp&orderby=DESC';
    var newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?stage=pipeline&searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery + sortby;
    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
    $.ajax({
...
function GetMyProjectContracts() {
    ...
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/myprojectcontractsinpipeline',
        type: 'GET',
        dataType: 'json',
        'Content-Type': 'application/json',
        cache: false,
...
function quickViewDisplay(obj) {
    ...
    ObjectNameToSend = obj.name;
    if (obj.name == "All") {
        colorLink('spnAllContractsInPipeline', true);
        colorLink('spnDraft', false);
        colorLink('spnWaitingOnMe', false);
        colorLink('spnOwnedByMe', false);
        colorLink('spnCreatedByMe', false);
    ...
    else if (obj.name == "Draft") {
        colorLink('spnAllContractsInPipeline', false);
        ...
        $("#divChkSelectAll").css('display', 'none');
        $("#btnFilter").css('display', 'none');
        $("#liFiltersforQuickViews").css('display', 'none'); //eO310581
    }
    savedViewNameFromViewTable = "";
    applyFilter();
    //showQuickViewFilter(obj.name);
...
function GetContractsWaitingOnMe() {
    $("#spResult").empty();
    var baname = "";
    if (thisBusinessAreaName == "" || thisBusinessAreaName == "All") {
    } else {
    ...
    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/waitingonme',
        type: 'GET',
        dataType: 'json',
        'Content-Type': 'application/json',
        cache: false,
...
function GetContractsOwnedByMe() {
    $("#spResult").empty();
    $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
    ...
    $.ajax({
        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/ownedbyme',
        type: 'GET',
        dataType: 'json',
        'Content-Type': 'application/json',
        cache: false,
        ...
        if (!allowcustomsearch) {
            ...
            switch (selectedSortOption) {
                ...
                case "Expiration Date":
                    ...
                    sortby = '&sortbyfield=ExpiryDate&orderby=' + defExpDate;
                    break;
                case "Contract Value(Low-High)":
                    if (typeof sortDirection != 'undefined' && sortDirection != "")
            ...
            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?BusinessAreaPath=' + baloc + '&stage=pipeline&searchkeyword=' + encodeURIComponent($("#txtSearchBox").val().trim()) + '&customquery=' + customquery + sortby + "&viewName=" + qvName;
            $("#txtSearchBox").val("");
            $('#listContracts').html('<img src="../Content/Images/icon/loading.gif">');
            $.ajax({
                url: newurl,
...
//            hasItem2 = true;
//        });
//        if ($("#filterContractType").find('option:selected').text() == "All") {
//            hasItem2 = false;
//        }
...
//        if (customQuery1 == "") {
//            customQuery1 = "Status:New,Drafting,Awaiting Review,Reviewed,Awaiting Approval,Approved,In Negotiation,Negotiation Complete,";
//        }


//        if (qvName == 'WaitingOnMe') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=Waiting on Me";
//        } else if (qvName == 'MyProjectContracts') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=My Project Contracts";
//        }
//        else if (qvName == 'MySavedDrafts') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=pipelinedraft";
//        } else if (qvName == 'OwnedByMe') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=Own by me";
//        } else if (qvName == 'CreatedByMe') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=Created by me";
//        } else if (qvName == 'RecentlyCreated') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=Recent Active Pipeline";
//        } else if (qvName == 'AwaitingSignature') {
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=";
//        } else if (qvName == 'AllContractsInPipeline' || (qvName == "")) {
//            if (isAnySelected1 || isAnySelected2) {
//                $("#btnAddView").css('display', 'block');
//            }
//            newurl = vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts?searchkeyword=' + encodeURIComponent($("#txtSearchBox").val()) + '&customquery=' + customQuery1 + customQuery2 + sortby + "&otherquery=&viewName=AllPipeline";
//        }
...
function liRemove(obj) {
    ...
    if ($(obj.parentNode.parentNode.parentNode).attr('id') == 'liFiltersSearchText') {
        ...
        var find = " ";
        var re = new RegExp(find, 'g');
        var valuestodisable = firstChild.nodeValue.replace(re, '');
        ...
        switch (firstChild.nodeValue) {
            case "Contract Owner":
                val = 'ContractManagers';
                break;
            case "Contract Manager":
                val = 'ContractManagers';
                break;
            case "Business Owner":
                val = 'BusinessAreaOwners';
                break;
            case "Creator":
                val = 'CreatedBy';
                break;
            ...
            case "Signee":
                ...
                break;
            case "Authorizer":
                val = 'RenewalConfirmParticipants';
                break;
        }

        $("#filterUserType option[value='" + val + "']").prop("selected", false);
        $("#filterStatus option[value='" + valuestodisable + "']").prop("selected", false);
        $("#filterContractType option[value='" + valuestodisable + "']").prop("selected", false);
    }
    var hasItem1 = false;
    $('#filterStatus :selected').each(function (i, selected) {
        hasItem1 = true;
    ...
    if (qvName == "All Contracts" && !hasItem1 && !hasItem2) {
        applyFilter();
    ...
    else if (qvName == "MyProjectContracts" && !hasItem1 && !hasItem2) {
        GetMyProjectContracts();
        $("#btnAddView").css('display', 'none');
    }
    else if (qvName == "MySavedDrafts" && !hasItem1 && !hasItem2) {
        GeContractsSavedAsDraft();
        $("#btnAddView").css('display', 'none');
    }
    else if (!hasItem7 && !hasItem2) {
        applyFilter();
        $("#btnAddView").css('display', 'none');
    }
    else {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (resultFound) {
                    ...
                    $('#liFiltersSearchText').html('<span><small>Showing search result for : ' + txtsearchboxvalue + '<img src="/Content/Images/close-quick.png" onclick="javascript:liRemove(this);" style="float:right" /></small></span>');
                    $("#listContracts").empty();
                    $("#listContracts").append("<label style='font-size:20px;'>No Result Found.</label>");
                }
                $("#compact-pagination").css('display', '');
            },
            error:
...
function checkMultipleContracts(object) {
    ...
    if (multipleChecks.trim() == "") {
        ...
        if (multipleChecksPermission.indexOf('openmenuView') >= 0) {
            ...
            if (typeof localStorage.GlobalBusinessAreaLocation != "undefined" && localStorage.GlobalBusinessAreaLocation != "" && localStorage.GlobalBusinessAreaLocation != "All")
                $("#btnMultipleAction_BusnArea").css('display', '');
            if (!(multipleChecksStatus.indexOf("New") >= 0 || multipleChecksStatus.indexOf("Awaiting Approval") >= 0 || multipleChecksStatus.indexOf("Approved") >= 0 || multipleChecksStatus.indexOf("Awaiting Review") >= 0 || multipleChecksStatus.indexOf("Reviewed") >= 0 || multipleChecksStatus.indexOf("In Negotiation") >= 0 || multipleChecksStatus.indexOf("Negotiation Complete") >= 0 || (multipleChecksIsDraftByCurrent.indexOf("No") >= 0)))
                $("#btnMultipleAction_Delete").css('display', '');
            else {
                $("#btnMultipleAction_Delete").css('display', 'none');
            }
...
function showStatusMultiple() {
    ...
    $('input:radio[name="PipelineStatusMul"]').attr('checked', false);
...
function changestatusmultiple() {
    var selectedValue = decodeURI($("input:radio[name=PipelineStatusMul]:checked").val());
...
function changestatusM() {
    if (requiredValidator("dvManageContractStatusMul", false)) {
        $("#loadingPage").fadeIn();
        var stat = decodeURI($("input:radio[name=PipelineStatusMul]:checked").val());
        $.ajax({
...
function MarkContractAsFinalMul() {
    swal({
        title: '',
        text: "Are you sure you want to <span style=\"font-weight:700\">mark</span> these contracts as <span style=\"font-weight:700\">Finalized/Ready for Signature</span>?",
        type: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes',
        cancelButtonText: 'No',
        ...
        function (confirmed) {
            if (confirmed) {
                var vdocs = $("#chkMarkDocAsFinalMul").is(':checked') ? 'Yes' : 'No';
                $.ajax({
                    url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Contracts/makefinalize?contractid=' + multipleChecks + '&finalized=Yes&username=' + localStorage.UserName + '&docs=' + vdocs,
                    type: 'PUT',
                    dataType: 'json',
                    headers: { 'eContracts-ApiKey': localStorage.APIKey, 'eContracts-RestrictionHeader': localStorage.RestrictHighSecurityTagging },
                    cache: false,
                    success: function (data) {
                        swal("", "Contract(s) record are sent for finalization. Check after some time in 'Contracts' tab.");
                        $("#dvManageContractStatusMul").dialog("close");
                        $("#loadingPage").fadeOut();
                        applyFilter();
                    },
        ...
        if ($("#hdWorkflowIsPublic").val() == "Yes") {
            ...
            $.ajax({
                ...
                success: function (UserList) {
                    ...
                    if (chkAutoUpdateStatus.checked) {
...
function ChangeCounterpartyMultiple() {
    ...
    if (vCoounterparty != "") {
        swal({
            title: '',
            text: "Are you sure you want to change counterparty?",
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes',
            cancelButtonText: 'No',
...
function GetValueAndPopulateManagerNew(contractItem, item) {
    ...
    //ENH-472 (In Pipeline, People & Permission tab display all the Global Contract Owners.)
    ...
    //        string += "<li id='PermissionList" + value + "'><div class='share-pop-up-Middle'><div class='Link_UserProfile'><p id='PermissionUser" + value + "' class='" + vUserClass + "' " + userTitle + ">" + user.UserName.trim() + "</p>";
    //        string += "<span id='UserRole" + value + "'>" + "(Global Contract Owner)" + "</span></div></div>";
    //        string += "<div class='share-pop-up-Right'><span class='sortSpanUserPermission' id='UserPermission" + value + "'><label>Full Control</label></span></div></li>";
        ...
        if (fullpermission) {
            ...
            if (strPermission == "Full Control" || strPermission == "Read/Write" || strPermission == "Read Only") {
                ...
                if (strPermission == "") {
                    ...
                    //contractRoleAccessUsers.push({ 'RoleName': itemfield, 'DisplayRoleName': itemfieldDis, 'Permission': Value, 'UserName': value.trim() });
                    //strUserRole
                    //manoj
                }
...
//    $.ajax({
//        url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/customlookupvaluedetails?lookupname=ContractTermType',
//        type: 'GET',
//        dataType: 'json',
//        "Content-Type": "application/json",
//        headers: { 'eContracts-ApiKey': localStorage.APIKey },
...
//                $("#ddlTermType").append("<option value='" + itemname + "'>" + itemname + "</option>")
//            }
//        }
//    });
//}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data != null && data.length != 0) {
                ...
                $('#tblPipelinePeople > tbody').append(html);
                $(ddllt).each(function () {
...
function BindContractRolesMetadata(Conitem) {
    ...
    $.ajax({
        ...
        success: function (mainmetadataFields) {
            ...
            //        control += '<td height="10" align="left" valign="top" class="content-text clr999 width40 ">' + itemfield + '</td>';
            ...
            //                control += ' <a href="javascript:void(0);" onclick="ViewUserProfile(\'' + this.toString().trim() + '\')" class="Link_UserProfile disabled_item_link">' + this.toString().trim() + '</a>; ';
            //            else
...
//ENH 452 Added Cancel Status in Pipeline & performing Cancel action
    ...
    if (requiredValidator('dvCancelContract', false)) {
     ...
     function (confirmed) {
         if (confirmed) {
             ...
             var vAbortNote = "CancelledReason=" + $("#txtReasonOfCancel").val() + "&UsersToNotify=" + noty;
             $.ajax({
...
function CancelContractMul() {
    if (requiredValidator('dvCancelContractMul', false)) {
        swal({
            title: '',
            text: "Are you sure you want to <span style=\"font-weight:700\">Cancel</span> this contract(s)?",
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Yes',
            cancelButtonText: 'No',
     ...
     function (confirmed) {
         if (confirmed) {
             ...
             var vAbortNote = "CancelledReason=" + $("#txtReasonOfCancelMul").val() + "&UsersToNotify=" + noty;
             $.ajax({
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(searchXml).find('Field').each(function (index, value) {
                    if ($.inArray($(value).find('FieldType').text(), notRequiredSearchFieldType) == -1) {
                        var objMetadata = {};
                        objMetadata.value = $(value).find('FieldDisplayName').text();
                        objMetadata.fieldType = $(value).find('FieldType').text();
                        objMetadata.choiceValues = $(value).find('ChoiceValues').text();
                        objMetadata.fieldName = $(value).find('FieldName').text();
                        objMetadata.label = objMetadata.value;
                        metadataLookUp.push(objMetadata);
                    }
                });
...
function GetUserPermissionIndex() {
    $.ajax({
...
function GetUniqueCopyLinkID() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#txtCopyLink').val(baseUrl + "Contracts/Link/" + data.replace(/[&\/\\#,+()$~%.'":*?<>{}]/g, 'a'));
