...
function getcontractareasettings(businessarename, contractareaname) {
    $.ajax({
...
function getRequestData() {
    ...
    $("#undercheck").empty();
    document.getElementById("btnCreateRequest").innerHTML = "Submit";
    $.ajax({
        ...
        success: function (mainmetadataFields) {
            ...
            var businessArea = $(vMetadata).find('BusinessArea').text();
            var contractArea = $(vMetadata).find('ContractArea').text();
            ...
            getcontractareasettings(businessArea, contractArea);
            var vCT = $(vMetadata).find('RequestType').text();
            getrequesttypes(vCT);

            if ($(vMetadata).find('IsDraft').text() == "Yes") {
                $("#btnSaveasDraftRequest").css('display', '')
                $("#divhelptext").css('display', '')
            }
            else {
                $("#btnSaveasDraftRequest").css('display', 'none');
                $("#divhelptext").css('display', 'none')
            }

            RequiredByDate = $(vMetadata).find('RequiredByDate').length > 0 ? $(vMetadata).find('RequiredByDate').text() : "";
            if (RequiredByDate != "") {
                RequiredByDate = moment(new Date(RequiredByDate)).format('MM/DD/YYYY');
            }
            $("#txtRequestBusinessArea").val($(vMetadata).find('BusinessArea').text());
            ...
            if ($(vMetadata).find('BusinessArea').text() != '') {
                ...
                IniBusinessAreaName = $(vMetadata).find('BusinessArea').text();
                IniRequesttype = $(vMetadata).find('RequestType').text();
            ...
            if (getParameterByName("Duplicate") == "Yes") {
                $("#txtRequestTitle").val($(vMetadata).find('RequestTitle').text() + "_Duplicate");
                $("#btnDuplicate").css('display', '');
                $("#btnCreateRequest").css('display', 'none');
            }
            else {
                $("#txtRequestTitle").val($(vMetadata).find('RequestTitle').text());
            }

            $.ajax({
                url: vApiBaseSiteUrl +'/api/accounts/' + localStorage.AccountID + '/Requests/requesttypes/metadatas?requesttypename=' + encodeURIComponent(vCT),
                type: 'GET',
                dataType: 'json',
                headers: { 'eContracts-ApiKey': localStorage.APIKey },
                cache: false,
                success: function (metadataFields) {
                    ...
                    $(metadataFields).each(function (i, item) {
                        ...
                        if ((item.FieldName == "RequestTitle") || (item.FieldName == "RequestType") || (item.FieldName == "BusinessArea") || (item.FieldType == "File Upload")) {
                            //manoj
...
//Get Request Types for the Edit Request
function getrequesttypes(seltype) {
    ...
    $.ajax({
        ...
        success: function (requesttypes) {
            $(requesttypes).each(function (i, item) {
                ...
                //    if (item.Active) {
                //        //triggerstatus = (typeof prvselectedreqtype != 'undefined' && prvselectedreqtype != null && prvselectedreqtype != "" && prvselectedreqtype != "--Select--" && triggerstatus == false) ? true : triggerstatus;
                //    }
                //} else {
                if (thisContractAreaSettings.RequestType.split(';').indexOf(item.RequestType) > -1) {
                    if (item.Active) {
                        if (prvselectedreqtype == item.RequestType) {
                            ...
                            //if($('#txtBA').val()!=IniBusinessAreaName && IniRequesttype !=
                            //if (treeBusinessAreaOwner != "") {
                            //    GetValuesAndAutoPopulate("AssignedTo", treeBusinessAreaOwner);
                            //}
                        } else {
            ...
            if (typeof prvselectedtrigger != 'undefined' && prvselectedtrigger != null && prvselectedtrigger != "" && prvselectedtrigger != $("#ddlRequestTypes option:selected").text()) {
                RequestTypeConfigure("Yes");
            } else {
                RequestTypeConfigure("No");
            }
            //manoj
        },
        error: function () {
...
function RequestTypeConfigure(CtypeAction) {
    //manoj
...
$("#ddlRequestTypes").change(function (obj) {
    ...
    //manoj
    //var vRequestTypeVal = $("#ddlRequestTypes option:selected").val().split('~');
    ...
    $.ajax({
        ...
        success: function (mainmetadataFields) {
            var vMetadata = $(mainmetadataFields).find('Metadata');
            var businessArea = ($("#txtRequestBusinessArea").val() != "" && $("#txtRequestBusinessArea").val() != null && typeof ($("#txtRequestBusinessArea").val()) != 'undefined') ? (($(vMetadata).find('BusinessArea').text() == $("#txtRequestBusinessArea").val()) ? $(vMetadata).find('BusinessArea').text() : $("#txtRequestBusinessArea").val()) : $(vMetadata).find('BusinessArea').text();
            var contractArea = ($("#txtContractAreaName").val() != "" && $("#txtContractAreaName").val() != null && typeof ($("#txtContractAreaName").val()) != 'undefined') ? (($(vMetadata).find('ContractArea').text() == $("#txtContractAreaName").val()) ? $(vMetadata).find('ContractArea').text() : $("#txtContractAreaName").val()) : $(vMetadata).find('ContractArea').text();
            var vCT = vRequestTypeVal[0];
            //$("#txtRequestBusinessArea").val(), $('#txtContractAreaName').val()
            getcontractareasettings(businessArea, contractArea);
            if ($(vMetadata).find('IsDraft').text() == "Yes") {
                $("#btnSaveasDraftRequest").css('display', '')
                $("#divhelptext").css('display', '')
            ...
            $("#txtRequestBusinessArea").val(businessArea);

            if (businessArea != '') {
                selecteditems = businessArea.split(',');
                $('#txtBA').val(businessArea);
                try {
                    $('#txtContractAreaName').val(contractArea);
                    $('#txtContractAreaAdministrators').val(($("#txtContractAreaAdministrators").val() != "" && $("#txtContractAreaAdministrators").val() != null && typeof ($("#txtContractAreaAdministrators").val()) != 'undefined') ? (($(vMetadata).find('ContractAreaAdministrators').text() == $("#txtContractAreaAdministrators").val()) ? $(vMetadata).find('ContractAreaAdministrators').text() : $("#txtContractAreaAdministrators").val()) : $(vMetadata).find('ContractAreaAdministrators').text());
                    $('#txtBusinessAreaOwners').val(($("#txtBusinessAreaOwners").val() != "" && $("#txtBusinessAreaOwners").val() != null && typeof ($("#BusinessAreaOwners").val()) != 'undefined') ? (($(vMetadata).find('BusinessAreaOwners').text() == $("#txtBusinessAreaOwners").val()) ? $(vMetadata).find('BusinessAreaOwners').text() : $("#txtBusinessAreaOwners").val()) : $(vMetadata).find('BusinessAreaOwners').text());
                    businessAreaPath = (businessAreaPath != "" && businessAreaPath != null && typeof (businessAreaPath) != 'undefined') ? (($(vMetadata).find('BusinessAreaPath').text() == businessAreaPath) ? $(vMetadata).find('BusinessAreaPath').text() : businessAreaPath) : $(vMetadata).find('BusinessAreaPath').text();
                }
                catch (err) { }
                var selecteditemslength = selecteditems.length;
                $('#liSelectedBA').empty();
...
function CheckGlobalSettingsEditRequest() {
        ...
        buttons: {
            "OK": function () {
                if ($('#liSelectedBA span').length > 0) {
                    ...
                    getcontractareasettings(strReplaced, $('#txtContractAreaName').val());
                    getrequesttypes("");
                    $(this).dialog("close");
                }
                else {
...
$('#btnClearFilterRequestEdit').click(function () {
...
$('#btnCreateRequest').click(function () {
    ...
    else { $("#spInProgress").css('visibility', 'hidden'); }
    autoscroll();
});


function UpdateRequest(isdraft) {
    if (typeof (isdraft) === 'undefined') isdraft = true;
    var vTitle = $("#txtRequestTitle").val().trim();
    var requestForm = $("#saveRequestForm *").serialize();

    //If multi dropdown is empty
    $('#saveRequestForm .multiselect').each(function (index) {
        if ($(this)[0].value == "") {
            requestForm += "&" + $(this)[0].id + "=";
        }
    });
    ...
    if (requestForm.indexOf("&OriginatingParty=") == -1) {
        if (requestForm.indexOf("&CompanyProfile=") == -1) {
            requestForm += "&OriginatingParty=" + thisContractAreaSettings.LegalEntity;
        } else {
    ...
    requestForm += "&BusinessArea=" + encodeURIComponent($("#txtRequestBusinessArea").val());
    requestForm += "&RequestType=" + encodeURIComponent($("#ddlRequestTypes option:selected").text().trim());

    if (businessAreaPath != "") { //if business area changed
        requestForm += "&BusinessAreaPath=" + encodeURIComponent(businessAreaPath);
        requestForm += "&ContractArea=" + $("#txtContractAreaName").val().trim();
        if ($("#txtContractAreaAdministrators").val() != null)
            requestForm += "&ContractAreaAdministrators=" + $("#txtContractAreaAdministrators").val().trim();
        if ($("#txtBusinessAreaOwners").val() != null)
            requestForm += "&BusinessAreaOwners=" + $("#txtBusinessAreaOwners").val().trim();
    }

    $('.multiselect').each(function () {
        var vmultiselectVal = $(this).val();
        var vmultiselectName = $(this).attr('name');
        if ((vv == '' || vv == null) && requestForm.indexOf("&" + vmultiselectName + "=") == -1) {
            requestForm += "&" + vmultiselectName + "=";
        }
    });
    ...
    if (RequestID != "") {
        requestForm += "&RequestID=" + getParameterByName('RequestID');
        $.ajax({
...
function Checkdatebeforsave() {
    if (!comparedates("StartDate", "EndDate", "End date should be greater than start date")) {
        ...
        if (RequiredByDate != "") {
            swal("", "Required by date has passed. Please change the Required by date.");
            RequiredByDate = "";
            $("#loadingPage").fadeOut();
        }
        else {
...
function treeviewclick2edit(obj) {
    ...
    treeBusinessAreaRowKey = obj.parentNode.parentNode.childNodes[1].textContent;
    treeBusinessAreaParentBusinessAreaID = obj.parentNode.parentNode.childNodes[0].textContent;
    treeBusinessAreaContractAreaName = obj.parentNode.parentNode.childNodes[2].textContent;
    treeBusinessAreaContractAreaNameOwner = obj.parentNode.parentNode.childNodes[3].textContent;
    treeBusinessAreaOwner = obj.parentNode.parentNode.childNodes[4].textContent;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(myCounterPartyArrayList).each(function (iArray, itemArray) {
                ...
                article += '<label for="CP' + itemArray.RowKey + '" class="css1-label" title="' + escape(itemArray.CounterpartyName) + '" onmouseover="UnescapeNameMouseOver(this)" style="display: inline;"><a href="/Counterparty/CounterpartyDetail?CounterpartyID=' + itemArray.RowKey + '" target="_blank">' + itemArray.CounterpartyName + '</a></label></td>';
                    ...
                    if ((item.FieldName == "CounterpartyName") || (item.FieldName == "CounterpartyType") || (item.FieldName == "Status")) {
                        ...
                        if (item.ShowInCreateForm == "true") {
                            ...
                            if (item.FieldType == "Single Line Text") {
                                ...
                                if (item.Required == "true") {
                                    vControls += "<select id=" + item.FieldName + " multiple='multiple' class='width90 multiselect chosenmulti validselect' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
                                } else {
                                    vControls += "<select id=" + item.FieldName + " multiple='multiple' class='width90 multiselect chosenmulti' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
                                }
                                ...
                                if (item.Required == "true") {
                                    vControls += "<select id=" + item.FieldName + " multiple='multiple' class='form-contro f_inpt width90 multiselect chosenmulti validselect' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
                                } else {
                                    vControls += "<select id=" + item.FieldName + " multiple='multiple' class='form-contro f_inpt width90 multiselect chosenmulti' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
                                }
        ...
        success: function (data) {
            ...
            if (datalength > 0) {
                for (var i = 0; i < datalength; i++) {
                    ...
                    article += '<label for="Pro' + item.RowKey + '" class="css1-label"><a href="javascript:ViewProject(\'' + escape(item.ProjectName) + '\')">' + item.ProjectName + '</a></label>';
            ...
            success: function (data) {
                ...
                if (datalength > 0) {
                    for (var i = 0; i < datalength; i++) {
                        ...
                        article += '<label for="' + vVarProjectName + item.RowKey + '" class="css1-label"><a href="javascript:ViewProjectTask(\'' + escape(vVarProjectName + ':' + item.TaskID) + '\')">' + item.TaskID + ' : ' + item.TaskDescription + '</a></label>';
                        article += '</td></tr>';
...
function AddContract() {
    ...
    vContractName = ContractArr.join(";");
    //manoj
    ...
    //        $('#RelatedContracts').val() = $('#RelatedContracts').val().trim().replace("; ", ";");
    //        splitselectedrelatedcontract = $('#RelatedContracts').val().split(";");
    //    }
...
function SearchContracts() {
    $("#tblContracts").html('');
    var selectedContractPrev = [];
    if ($("#liSelectedRelatedContract").html() != '') {
        var tempData = $('div#liSelectedRelatedContract > span');
        $(tempData).each(function (i, item) {
            var obj = {};
            obj.id = item.id;
            obj.title = item.innerText;
            selectedContractPrev.push(obj);
        });
    }
        ...
        success: function (data) {
            ...
            for (var i = 0; i < datalength; i++) {
                ...
                if (arr.indexOf(item.ContractTitle) >= 0 || arrCurrentRelated.indexOf(item.ContractTitle) >= 0) {
                    bindlist = true;
                    article += '<input id="Pro' + item.RowKey + '" type="checkbox" name="RelatedContracts" class="css1-checkbox" onchange="javascript:collectrelatedcontractrowkey(this);" checked value="' + item.ContractTitle + '" />';
                } else {
                ...
                article += '<label for="Pro' + item.RowKey + '" class="css1-label PreserveSpace"><a href="/Contracts/ContractDetails?ContractID=' + item.RowKey + '" target="_blank">' + item.ContractTitle + '</a></label></td><td><label>' + item.ContractType + '</label></td><td><label>';
                ...
                if (bindlist) {
                    articleselctedList += '<span style="font-size:11px;" id="Pro' + item.RowKey + '">' + item.ContractTitle + '<img src="/Content/Images/close-quick.png" onclick="javascript:liRemoveRelatedContract(this);" style="float:right"></span>'
                    selectedContractPrev = selectedContractPrev.filter(function (obj, index) {
                        return obj.id != 'Pro' + item.RowKey
                    })
                }
            }
            ...
            articleselctedList = '';
            $(selectedContractPrev).each(function (i, item) {
                articleselctedList = '<span style="font-size:11px;" id=' + item.id + '>' + item.title + '<img src="/Content/Images/close-quick.png" onclick="javascript:liRemoveRelatedContract(this);" style="float:right"></span>'
                $("#liSelectedRelatedContract").append(articleselctedList);
            });
...
function createingdivbyme(item, vControlsGroup, idelvalue, idelcheck, vUserList, vMetadata) {
    ...
    if (item.FieldType == "Single Line Text") {
        ...
        if (item.Required == "true") {
            ...
            vControls += "<input type='text' id=" + item.FieldName + "  class='form-contro form-contro-Date validdate form_input fielddatecontrol " + item.FieldName + "'/>";
        }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < datalength; i++) {
                        ...
                        if ($(vMetadata).find(item.FieldName).text() == data[i].ContractTermName) {
        ...
        vControls += '<div class="col6 m12">';
        if (item.Required == "true") {
            vControls += "<select id=" + item.FieldName + " multiple='multiple' class='form-contro multiselect chosenmulti validselect' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
        } else {
        ...
        if (item.FieldName == "Counterparty") {
            ...
            if (item.Required == "true") {
                vControls += "<input type='text' id=" + item.FieldName + " name=" + item.FieldName + " value='" + $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39") + "' class='form-contro validelement' " + vreadonly + " type='text' style='display:none' />";
                if ($(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39").indexOf(';')) {
                    splitCounterParty = $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39").split(';');
                    vControls += "<ul id='counterpartyUL'>";
                    $(splitCounterParty).each(function (i, item) { //Bug id :eO37627
                        var itemCounterparty = item.replace(/\s+/g, '');//Remove white spaces from string
                        if (AllCounterparties.length > 0) {
                        ...
                        if (itemCounterparty != '') {
                            if (vCounterparty != null && vCounterparty != "" && vCounterparty.length > 0)
                                vControls += '<li id="' + escape(itemCounterparty) + '"  class="clsCounterparties" style="margin: 0 0 5px 0;"><div style="display: inline-block;">' + '<a>' + item + '</a>' + '</div><div style="display: inline-block;padding-left: 10px;"><a href="javascript:void(0)" class="linkText" onclick="ViewCounterpartyDetails(\'' + itemCounterparty + '\')"> View Details</a></div>' + '  <a href=javascript:void(0)  >' + '<img src="/Content/Images/icon/delete.png" onclick="removeCounterParty(this)" title="Remove">' + '<a>' + '</li>';
                            else
                                vControls += '<li id="' + escape(itemCounterparty) + '" class="clsCounterparties" style="margin: 0 0 5px 0;"><div style="display: inline-block;">' + '<a style="color: #6a6a6a !important;">' + item + '</a>' + '</div><div style="display: inline-block;padding-left: 10px;"><a href="javascript:void(0)" class="linkText" onclick="ViewCounterpartyDetails(\'' + itemCounterparty + '\')"> View Details</a></div>' + '  <a href=javascript:void(0)  >' + '<img src="/Content/Images/icon/delete.png" onclick="removeCounterParty(this)" title="Remove">' + '<a>' + '</li>';
                        }
                    ...
                    vControls += "<ul id='counterpartyUL'>";
                    //vControls += '<li id=' + $(vMetadata).find(item.FieldName).text() + ' class="clsCounterparties" style="margin: 0 0 5px 0;"><div style="display: inline-block;">' + '<a>' + $(vMetadata).find(item.FieldName).text().trim() + '</a>' + '</div><div style="display: inline-block;padding-left: 10px;"><a href="javascript:void(0)" class="linkText" onclick="ViewCounterpartyDetails(\'' + $(vMetadata).find(item.FieldName).text().trim() + '\')"> View Details</a></div>' + '  <a href=javascript:void(0)  >' + '<img src="/Content/Images/icon/delete.png" onclick="removeCounterParty(' + '\'' + $(vMetadata).find(item.FieldName).text().trim() + '\'' + ')" title="Remove">' + '<a>' + '</li>';
                    vControls += "</ul>";
                }

            } else {
                vControls += "<input type='text' id=" + item.FieldName + " name=" + item.FieldName + " value='" + $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39") + "' class='form-contro' " + vreadonly + " type='text' style='display:none' />";
                if ($(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39").indexOf(';')) {
                    splitCounterParty = $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39").split(';');
                    vControls += "<ul id='counterpartyUL'>";
            ...
            vControls += '<div class="col6 m12">';
            if (item.Required == "true") {
                vControls += "<select id=" + item.FieldName + " name=" + item.FieldName + " class='form-contro validelement'>";
            } else {
                vControls += "<select id=" + item.FieldName + " name=" + item.FieldName + " class='form-contro'>";
            }
            //vControls += "<option value='0'>--Select--</option>";
        ...
        if (item.FieldType.indexOf("Dropdown") > -1) {
            ...
            if (item.Required == "true") {
                ...
                vControls += "<select id=" + item.FieldName + " multiple='multiple' class='form-contro  multiselect chosenmulti' name=" + item.FieldName + " title='" + item.FieldDisplayName + "' data-placeholder='--Select--'>"
            }
            var vMultiDDL1 = "";
            if (vMultiDDL1 == '') {
                var myArray = [];
            ...
            if (item.ChoiceValues == "Counterparty") {
                ...
                var recounterparty = new RegExp("'", 'g');
                if (item.Required == "true") {
                    vControls += "<input type='text' id=" + item.FieldName + " name=" + item.FieldName + " value='" + $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39") + "' class='form-contro validelement' " + vreadonly + " type='text' />";
                } else {
                    vControls += "<input type='text' id=" + item.FieldName + " name=" + item.FieldName + " value='" + $(vMetadata).find(item.FieldName).text().replace(recounterparty, "&#39") + "' class='form-contro' " + vreadonly + " type='text' />";
                }
                vControls += '<label class="col12 p-text text-left help">' + item.Description + '</label>';
                vControls += '</div>';
                vControls += '<div class="col3 m1">';
    ...
    if (vControlsGroup != "") {
        ...
        if (vProject) {
            ...
            if (item.Required == "true") {
                ...
                vControls += '<div class="col6 m11">';
                vControls += "<input id='ProjectTask' name='ProjectTask' title='Project Task' value='" + $(vMetadata).find("ProjectTask").text() + "' class='form-contro validelement' readonly='readonly' type='text' />";
            }
            else {
                vControls += '<label id="lblProjectTask" class="col3 m12 margin-top-8">Project Tasks</label> ';
                vControls += '<div class="col6 m11">';
                vControls += "<input id='ProjectTask' name='ProjectTask' title='Project Task' value='" + $(vMetadata).find("ProjectTask").text() + "' class='form-contro' readonly='readonly' type='text' />";
            }
            vControls += '</div>';
            vControls += '<div class="col3 m1">';
            vControls += '<div class="success-input-msg margin-top-8">';
...
function CreateCounterPartyListUnit(page) {
    ...
    if (startIndex == endIndex) {
        ...
        for (var i = startIndex; i < endIndex; i++) {
            ...
            article += '<label for="CP' + myCounterPartyArray[i].RowKey + '" class="css1-label"  style="display: inline;"><a href="/Counterparty/CounterpartyDetail?CounterpartyID=' + myCounterPartyArray[i].RowKey + '" target="_blank">' + myCounterPartyArray[i].CounterpartyName.trim() + '</a></label></td>';
...
function comparedateswithGivenDate(FirstDate, secondDateControlID) {
    var isvalid = true;
...
function BindBusinessAreMenuCounterp(data) {
    if (typeof (BusinessAreaAccessWithRead) == "undefined" || BusinessAreaAccessWithRead == null || BusinessAreaAccessWithRead.length == 0) {
        ...
        if (typeof (BusinessAreaAccessWithRead) == "object" && BusinessAreaAccessWithRead.length > 1) {
            BusinessAreaAccessCounterp = BusinessAreaAccessWithRead;
        }
        else
            BusinessAreaAccessCounterp.push(BusinessAreaAccessWithRead);
...
function liRemoveRelatedContract(obj) {
    ...
    if ($('#RelatedContracts').val().indexOf(relatedContractTitle) > -1) {
        child.parentNode.removeChild(child);
        ...
        splitselectedrelatedcontract = jQuery.grep(splitselectedrelatedcontract, function (a) {
            return a !== relatedContractTitle.trim();
        });
...
function collectrelatedcontractrowkey(obj) {
    if (obj.checked) {
        ...
        if ($('#RelatedContracts').val().indexOf(obj.value) > -1) {
...
function CreateRelatedContractsList(page) {
    ...
    if ($("#RelatedContracts").val() != "") {
        if ($("#RelatedContracts").val().indexOf(';') >= 0)
            arrCurrentRelated = $("#RelatedContracts").val().split(';');
        else
            arrCurrentRelated.push($("#RelatedContracts").val())
    }
    $("#tblContracts").empty();
    var startIndex = page * 10;
    var endIndex = startIndex + 10;
    ...
    for (var i = startIndex; i < endIndex; i++) {
        ...
        var bindlist = false;
        var article = '<tr><td>';
        if ($('#RelatedContracts').val().indexOf(item.ContractTitle) >= 0 || arrCurrentRelated.indexOf(item.ContractTitle) >= 0) {
            if (parseInt($("#liSelectedRelatedContract").children().length) > 0) {
            ...
            article += '<input id="Pro' + item.RowKey + '" type="checkbox" name="RelatedContracts" onchange="javascript:collectrelatedcontractrowkey(this);" class="css1-checkbox" checked value="' + item.ContractTitle + '" />';
        } else {
            article += '<input id="Pro' + item.RowKey + '" type="checkbox" name="RelatedContracts" onchange="javascript:collectrelatedcontractrowkey(this);" class="css1-checkbox" value="' + item.ContractTitle + '" />';
        }
        article += '<label for="Pro' + item.RowKey + '" class="css1-label PreserveSpace"><a href="/Contracts/ContractDetails?ContractID=' + item.RowKey + '" target="_blank">' + item.ContractTitle + '</a></label></td><td><label>' + item.ContractType + '</label></td><td><label>';
        if (item.Counterparty != null && item.Counterparty != "") {
            article += item.Counterparty
    ...
    var addedrelatedContractArr = $.map($('#RelatedContracts').val().split(';'), $.trim);
    ContractArr = [];
    if (parseInt($("#liSelectedRelatedContract").children().length) > 0) {
        $("#liSelectedRelatedContract").children().each(function (i, n) {
            ContractArr.push(n.innerText);
    ...
    var SelectedRelContractID = $.grep(listRelatedContracts, function (nRelConID, indRelConID) {
        return (addedrelatedContractArr.indexOf(nRelConID.ContractTitle) >= 0);
    });
...
function Loading_View_trigger() {
    CheckGlobalSettingsEditRequest();
