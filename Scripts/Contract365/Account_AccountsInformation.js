...
$(document).ready(function () {  
    strPartionKey = getParameterByName('AccountKey');
    ...
    $("#addEditAccountResources").dialog({
        autoOpen: false,
        closeText: "",
        width: "70%",
        height: "auto",
        ...
        buttons: {
            "Save": function () {
                if (requiredValidator("addAccountResources",false)) {
                    $.ajax({
                        url: vApiBaseSiteUrl + '/api/accounts/' + strPartionKey + '/LicenseLimit',
                        type: 'PUT',
                        dataType: 'json',
                        ...
                        success: function (person) {
                            $("#loadingPage").fadeOut();
                            location = "/Accounts/AccountsInformation?RowKey=" + person.RowKey + "&AccountKey=" + person.PartitionKey;
    ...
    BindResources(strPartionKey);

    $('#txtPartitionKey').val(strPartionKey)
    $('#txtrowkey').val(strPartionKey)
});
...
function SaveAccountSettings() {
    var currenewaldate = $("#txtContractTermEvaluationDate").val();
    var curstatus = $("#ddlIndustryOrg").find('option:selected').text();
    var activedt = $("#txtDate12").val();
    if (!comparedates("txtDate12", "txtContractTermEvaluationDate", "Renewal date should be greater than Activation date")) {

        alert("Renewal date should be greater than Activation date");

    }
    else {

        $.ajax({
            url:vApiBaseSiteUrl + '/api/accounts/' + strPartionKey + '/LicenseLimit?status=' + curstatus + '&RenewalDate=' + currenewaldate,
            type: 'PUT',
            dataType: 'json',
...
function BindResources(AccountID) {
    $.ajax({
        url: vApiBaseSiteUrl + '/api/accounts/' + AccountID + '/LicenseLimit',
        type: 'GET',
        dataType: 'json',
        ...
        success: function (entity) {
            accountLicenseLimit = entity;
            if (entity.CompanyName == "" || entity.CompanyName == null) {
               $('#headingCompanyName').text("Not Available");
            }
            else {
            ...
            if (output != "" && $("#txtContractTermEvaluationDate").val() != "") {
                ...
                var To_date = new Date($("#txtContractTermEvaluationDate").val());
            ...
            ContractsforContractAreaLimitArray = [];
            ContractsforContractAreaUsedArray = [];
...
function Clearclass()
{
    $('#txtContractAreas').removeClass('error');
    $('#txtContractsperContractAreas').removeClass('error');
    $('#txtContracts').removeClass('error');
    $('#txtUsers').removeClass('error');
    $('#txtBusinessUser').removeClass('error');
    $('#txtContractAdministrator').removeClass('error');
    $('#txtGlobalContractOwner').removeClass('error');
    $('#txtAccountOwner').removeClass('error');
    $('#txtBusinessAreaOwner').removeClass('error');
    $('#txtContractTypes').removeClass('error');
    $('#txtActiveContractTypes').removeClass('error');
    $('#txtCounterparties').removeClass('error');
    $('#txtActiveCounterparties').removeClass('error');
    $('#txtActiveNegotiationPortals').removeClass('error');
}

function EditResorces() {
    
    if (accountLicenseLimit != null && accountLicenseLimit != "") {
        $("#txtRowKey").val(accountLicenseLimit.RowKey);
        $("#txtPartitionKey").val(accountLicenseLimit.PartitionKey);
        $("#txtContractAreas").val(accountLicenseLimit.ContractAreaLimit);

        if (ContractsforContractAreaLimitArray.length > 0) {

            for (key in ContractsforContractAreaLimitArray) {

                if (ContractsforContractAreaLimitArray[key] != null && ContractsforContractAreaLimitArray[key] != "") {
                    $("#txtContractsperContractAreas").val(ContractsforContractAreaLimitArray[key]);
                    break;
                }
            ...
            ContractsforContractAreaLimitArray = getXMLToArray(accountLicenseLimit.ContractsforContractAreaLimit);
        ...
        $("#txtContracts").val(accountLicenseLimit.ContractsLimit);
        $("#txtUsers").val(accountLicenseLimit.CoreUsersLimit);
        $("#txtBusinessUser").val(accountLicenseLimit.BusinessUserLimit);
        $("#txtContractAdministrator").val(accountLicenseLimit.ContractAdministratorLimit);
        $("#txtGlobalContractOwner").val(accountLicenseLimit.GlobalContractOwnerLimit);
        $("#txtAccountOwner").val(accountLicenseLimit.AccountOwnerLimit);
        $("#txtBusinessAreaOwner").val(accountLicenseLimit.BusinessAreaOwnerLimit);
        $("#txtContractTypes").val(accountLicenseLimit.ContractTypesLimit);
        $("#txtActiveContractTypes").val(accountLicenseLimit.ActiveContractTypesLimit);
        $("#txtCounterparties").val(accountLicenseLimit.CounterpartiesLimit);
        $("#txtActiveCounterparties").val(accountLicenseLimit.ActiveCounterpartiesLimit);
        $("#txtActiveNegotiationPortals").val(accountLicenseLimit.ActiveNegotiationPortalsLimit);
        Clearclass();
        $("#addEditAccountResources").dialog("option", "title", "Edit Account Resources");
        $("#addEditAccountResources").dialog("open");
        $("#addEditAccountResources").height("auto");
    }
    else {
        $("#txtContractAreas").val("0");
        $("#txtContractsperContractAreas").val("0");
        $("#txtContracts").val("0");
        $("#txtUsers").val("0");
        $("#txtBusinessUser").val("0");
        $("#txtContractAdministrator").val("0");
        $("#txtGlobalContractOwner").val("0");
        $("#txtAccountOwner").val("0");
        $("#txtBusinessAreaOwner").val("0");
        $("#txtContractTypes").val("0");
        $("#txtActiveContractTypes").val("0");
        $("#txtCounterparties").val("0");
        $("#txtActiveCounterparties").val("0");
        $("#txtActiveNegotiationPortals").val("0");
        Clearclass();
        $("#addEditAccountResources").dialog("option", "title", "Edit Account Resources");
        $("#addEditAccountResources").dialog("open");
        $("#addEditAccountResources").height("auto");
